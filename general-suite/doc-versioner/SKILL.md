...
name: doc-versioner
version: 1.1.0
description: 文档版本化出版工具——基于源 Markdown 输出版本化 .md 快照，支持模板初始化、单文档/profile/manifest 批量生成、旧快照自动归档。当用户提到"生成文档快照""版本化发布文档""批量出版文档""初始化文档模板""维护修订记录""归档旧快照"时，应使用本 skill。注意：本 skill 负责"文档出版/版本化"，不负责"PRD内容写作"；PRD内容写作请使用 pd-prd-writing skill。
governance_id: "S-011"
triggers:
  - 文档版本化出版工具
  - 输出版本化
...
# doc-versioner · 文档版本化出版工具

> Track: `S`（共用层）| Phase: `P4`（执行）| Sub-position: `P4-a 文档交付`
> Issue Type: `DELIVERY_PUBLISHING`
> Knowledge Sources: [Markdown 版本化规范, 等保2.0 三级合规基线]
...
本 skill 专注**文档出版/版本化**，当用户出现以下意图时使用（写 PRD 内容请使用 pd-prd-writing skill）：

- 生成文档快照 / 版本化发布文档 / 出版 Markdown 文档
- 按模板初始化文档源文件
- 修改现有源文档并输出新版本快照
- 批量生成多个文档快照
- 为某项目建立通用 profile / manifest
- 将 Markdown 文档归档为可交付的版本化 `.md` 快照
- 维护文档修订记录与版本号
...
| 类型 | 字段名 | 格式/说明 | 来源 |
|:----:|:------|:---------|:-----|
| [M] | source_file | 绝对路径，无版本号源 .md 文件 | 用户直接提供 |
| [M] | output_dir | 输出目录绝对路径 | 用户直接提供或默认源文件同目录 |
| [O] | version | 版本号（如 v0.2），缺省则从源文件文档信息表读取，再缺省为 v0.1 | 用户提供或自动获取 |
| [O] | profile | profile 文件路径，提供公共字段默认值 | 用户可选提供 |

### Tools & Techniques（处理逻辑 — 可追溯）

| 步骤 | 方法 | 标准出处 | 说明 |
|:---:|------|:--------|------|
| 1 | 读取源 Markdown 文件 | Markdown 解析规范 | 提取文档信息表中的版本号 |
| 2 | 生成带版本号快照文件 | 文件命名规范 `{baseName}_{version}.md` | 版本号格式 `v主.次.修订` |
| 3 | 归档旧快照 | 文件移动规范 | 旧文件移入 `backup/` 目录 |
| 4 | 校验源文件与快照内容一致性 | Markdown 差异对比 | 确保出版质量 |

**Branch Conditions（场景分支）：**

| 条件 | 触发时机 | 处理差异 |
|:----|:--------|:--------|
| profile 存在 | 用户提供了 --profile 参数 | 先合并 profile 字段到 CLI 参数，再执行生成 |
| manifest 存在 | 用户提供了 --manifest 参数 | 按 manifest 逐条处理，每条可叠加 profile |
| 无版本号 | 源文件文档信息表缺少"文档版本"字段 | 默认使用 v0.1，并在源文件补填 |

### Output（输出 — 有验收标准）

| 类型 | 输出项 | 格式 Schema | 验收标准 |
|:----:|:------|:----------|:--------|
| 主要 | 版本化快照 `.md` 文件 | Markdown 文件 `{baseName}_{version}.md` | 文件存在且内容与源文件一致 |
| 附加 | 归档旧快照 | 旧文件位于 `backup/` 目录 | 旧版本文件已移入 backup/ |
| 附加 | 无版本号源文件 | 与快照内容一致，不含版本号 | 源文件存在，版本号与快照匹配 |

### Boundary（边界声明）

| 方向 | 声明 |
|:---:|:-----|
| **不做** | 不撰写 PRD/需求文档的业务内容；不做业务需求分析；不替用户决策功能范围 |
| **不追问** | 不追问项目业务逻辑细节；不追问用户个人隐私信息 |
| **不收口** | 若用户意图是"写 PRD 内容"，handoff 到 `pd-prd-writing` skill |
| **下游** | `pd-prd-writing`（PRD 内容撰写完成后，使用本 skill 出版版本化快照） |
