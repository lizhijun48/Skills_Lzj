...
**治理升级：GOV v3.6→v3.7（原则十一·客观多视角评估 + 治理自反触发条款）+ 新建 S-072 外部来源多视角评估 SOP**

### 一、背景

SRC-004 引入复盘时用户追问：原评估报告自述视角为"对照 Alex 现有技能治理体系的差距与可借鉴点"，是否片面？核查确认该视角属**单一吸收视角**（只问"对我有无用 / 能否填洞"），存在 7 类盲区（技术正确性未独立核验 / 无外部基准 / 融合二阶风险低估 / 使用者价值缺位 / 生命周期 TCO 缺位 / 治理自反缺位 / 来源许可与战略契合未评）。用户拍板按建议落地三件套。

### 二、交付物

1. **GOV_SkillGovernance.md v3.6→v3.7**
   - §10 新增**原则十一：客观多视角评估（引入前置）**——外部来源入库前禁止单一吸收视角，强制 8 镜头评估，结论记 SOURCE-REGISTRY，未过评不得入库
   - 原则十一内嵌**治理自反（元治理）触发条款**——镜头⑧标 GOV 盲点 / 每 +20 技能或 6 个月 / 合并拆分致可发现性回归 → 触发 GOV 自反评审，产出记 DECISION-LOG + CHANGELOG，复用既有四记录，**不新建平行元治理系统**
   - 后续治理计划：10 条原则 → 11 条，修正原第 5 条重复编号，新增"原则十一即时生效"
2. **新建 S-072 `external-source-eval-sop/`**（根目录独立部署，S-071 方法论兄弟）——8 镜头逐项判定口径 + 6 步执行流程 + 结论模板 + 反模式；明确"任一 ❌ 即不过评，非加权求和"

### 三、治理同步

- SKILL-ID-REGISTRY.md v1.5.9→v1.6.0（S-072 行 + 归属判定）
- SKILL-CATALOG.md 141→**142** 条目
- README.md 治理文件表 v3.6→v3.7、原则一~十→原则一~十一
- DECISION-LOG.md 新增治理升级决策条目

### 四、配套动作

- 用 S-072 对 SRC-004 做回头看复评，补齐原报告缺失的 ①③⑤⑥⑦⑧ 镜头（见 DECISION-LOG）
...
用户复盘需求："引入过哪些外部文件、对哪些技能做了融入与优化"。现有记录回答不了——CHANGELOG 是时间/版本视角（要查某源文件的流向需翻遍全文件）；技能级 `references/source-material*.md` 分散、无全局视图（一份材料喂多技能时无法一眼看清）。

### 二、新建 `SOURCE-REGISTRY.md`

按来源视角登记，每行一个引入事件：源ID（SRC-xxx）/ 来源文件 / 类型 / 引入日期 / **版本锚点**（只放 CHANGELOG 版本号+commit 指针，不重复细节）/ 流向技能（编号+融入位置）/ 融入方式 / 复盘备注。

**回填 3 条历史引入**（均有据可查）：

| 源ID | 来源 | 流向 |
|------|------|------|
| SRC-001 | 清华《DeepSeek-R1从入门到精通》PDF | S-026~S-031 + S-034~S-039 共 12 技能（含 S-027 提示语链七大作用机制、S-028 元叙事框架） |
| SRC-002 | 《DeepSeek 使AI变得简单》 | S-029 v1.3 + source-material-deepseek.md（约70%重叠，仅取2个增量知识点） |
| SRC-003 | AI编程课程培训材料 | 6 个技能的 references/source-material 素材存档 |

### 三、治理接入

- `README.md` 治理文件表新增 SOURCE-REGISTRY 行；"三文件分工"扩为"四记录分工"
- 维护规则写入文件头：引入动作即时登记并入 commit；二次利用更新原行锚点
...
- 版本变更历史（v0.1.0~v1.6.3，约 400 行）整体迁出至 `CHANGELOG.md`
- 顶层 `README.md` 保留：仓库定位 / 仓库结构 / 套件级 README 索引 / 治理文件 / 变更入口

### 二、补齐套件级 README 索引（方案A）

按「产品轨 / 项目轨 / 共用层 → 套件 → 技能」二级结构，只补套件级索引，不动 SKILL.md。

- 新增 11 个缺失套件 README：pd-suite / pm-suite / meta-suite / legal-suite / economic-suite / market-suite / expert-suite / gaoxiang-suite / reading-os / resume-optimizer / insurance-policy-analysis
- 修正 3 个过时套件 README：general-suite（"未治理"区改为已纳管 S-051~S-069）、industry-suite（编号"未分配"改为 I-001~I-006）、research-suite（编号改为 RE-001~RE-003）
- gridman README 顶部标注编号 S-019 + 更名状态（不动其产品文档与角色声明）
- 统一模板：套件定位 / 技能清单表（编号+技能+阶段+职责）/ 协作关系 / 数据基础设施标注，与 `SKILL-ID-REGISTRY.md` v1.5.6 同源

> 关键认知：每个技能已有 `SKILL.md`（=叶子级文档），真正缺失的是"套件级索引入口"。本次补的正是这一层。

---

## v1.6.2 — 2026-08-31

**新建 legal-suite/law-repository 中央法规库：法律内容单一事实来源（数据基础设施，不占编号）**

提交：本次 | 变更：新建 law-repository/ 四件套 + SKILL-CATALOG.md 基础设施条目 + DECISION-LOG.md 决策条目

### 一、库结构

| 组件 | 内容 |
|------|------|
| laws/ | 法条原文快照，一部法一 .md，YAML frontmatter 强制版本戳（版本/施行日期/快照日期/来源/状态/规模） |
| sources/ | 原始来源归档（招投标采购法规汇编PDF 1.3MB） |
| law-manifest.md | 已入库/待补/待核三区清单，唯一盘点入口 |
| README.md | 更新规程六条（人工触发/三粒度/180天阈值/git一更新一commit/级联标记/优先级链） |

### 二、种子入库（4部）

招标投标法2017修订（6章68条）/ 政府采购法2014修正（9章88条）/ 招标投标法实施条例（版本待核实，7章85条）/ 政府采购法实施条例2015施行（9章79条）——pypdf 页面切片提取 + 边界修复（第67/68条误切已迁回）+ 规模校正

### 三、已知限制

- flk.npc.gov.cn API 改版，在线核验通道暂不可用 → 民法典（P0）/保险法（P1）等按 manifest 待补清单后续入库
- 机器提取文本存在少量 reflow 合并瑕疵（章标题与首条合并），不影响检索
- 下游技能（pm-bid-proposal/contract-review 等）引用处补溯源标注为后续治理动作

---

## v1.6.1 — 2026-08-31

**治理框架 v3.6：用户口述底层逻辑固化为三条新原则（修改前置判定 / 模块化拆分融合 / 外部技能通用化）**

提交：待推送 | 变更：GOV_SkillGovernance.md v3.5→v3.6 + DECISION-LOG.md 新增决策条目

### 一、三条新原则（§10，原则八/九/十）

| 原则 | 核心规则 | 生效方式 |
|------|---------|---------|
| 原则八·修改前置治理判定 | 任何技能修改前必须输出"严格符合 / 特殊情况偏离"二选一判定；改治理原则 vs 改技能由人类裁决 | **即时生效** |
| 原则九·模块化拆分与融合 | 超大技能走模块化+流程化路径；模块须可经业务流程组合调用；功能相近模块必须评估融合（对齐 QG2） | 瘦身执行时生效 |
| 原则十·外部技能通用化 | 第三方技能拆分时个性化名称/显著标识通用化；原版归档保留溯源；fork 决策记 DECISION-LOG | 拆分触发时生效 |

### 二、连带变更

- 治理计划由 7 条原则更新为 10 条；原则八即时生效条款写入"后续治理计划"
- DECISION-LOG.md 新增 2026-08-31 决策条目（含影响面：三文件瘦身方案须升级为 v2，补治理判定列/融合评估/通用化范围）
- 待执行：gridman（"真寻Charlotte"人设）、law-skills、pm-bid-proposal 瘦身按新原则出 v2 方案
...
**对标结果**（详见 `DECISION-LOG.md` "能力对标分析"章节）：

| 评估场景 | 达到的水平 | 说明 |
|----------|------------|------|
| **企业内部立项初审** | ✅ **优秀** | 可替代 70% 的初级分析师工作 |
| **天使轮/Pre-A轮投资初筛** | ✅ **良好** | 可完成 60% 的尽职调查清单 |
| **A轮及以上领投方IC汇报** | ⚠️ **勉强** | 缺少完整尽调和单位经济分析 |
| **央企/国企重大项目评审** | ✅ **达标** | 可满足政策/技术/经济效益测算要求 |
| **政府产业基金投决** | ⚠️ **勉强** | 需补充产业化能力和退出机制 |
| **投行IPO/并购估值** | ⚠️ **部分达标** | DCF和可比公司已支持，缺少LBO模型 |

**核心优势**：结构化思维强、概率锚定科学、期望值理念先进、乘性零因子创新、贝叶斯先验回写、敏感性分析完备、可比公司对标

**核心短板**：缺少尽调验证、缺少单位经济分析、缺少退出路径规划、缺少蒙特卡洛模拟、缺少团队尽调辅助

### 六、治理文件更新

| 文件 | 变更 |
|------|------|
| `memory/MEMORY.md` | 新增 pd-go-nogo 升级成果索引 + 决策日志索引 |
| `memory/project-pd-go-nogo-enhancement-2026-08-31.md` | 新建详细记录文件（背景/内容/验证结果/下一步建议） |
| `DECISION-LOG.md` | 新建决策日志文件（改进建议/优先级/实施状态/历史决策/能力对标分析） |

**文件对照关系**：
- `README.md`：记录实际版本修改历史（What was changed）
- `DECISION-LOG.md`：记录改进建议和决策背景（Why it was changed + What's next）
- 两者相互引用，形成完整的演进轨迹
