...
| **Version**          | `v3.7`                                                       | 新增原则十一（客观多视角评估·引入前置）+ GOV 自反评审触发条款（元治理） |
...
### 原则十一：客观多视角评估（引入前置 / Objective Multi-View Evaluation）

任何外部来源（marketplace 导入、第三方作者署名、带 IP 声明、fork、SRC 系列外部来源）**引入入库前**，禁止仅以"对我有无用 / 能否填洞"的单一吸收视角决策；须站客观 / 上帝视角按多维度评估，结论记入 SOURCE-REGISTRY 引入流程，未过评不得入库。

**强制 8 镜头**（与 `S-072` 外部来源多视角评估 SOP 一一对应）：

| #   | 镜头                 | 要回答的问题                                                   | 不做的后果         |
| :-- | :------------------- | :------------------------------------------------------------- | :----------------- |
| ①   | 技术正确性独立核验   | 公式 / 模型 / 方法是否为业界标准口径？有无权威出处？           | 误导性方法论入库   |
| ②   | 外部基准对比         | 是否有可对照的独立基准或同类样本？单一样本结论是否可信？       | 单点偏差           |
| ③   | 融合二阶风险         | 接入后对我方耦合度 / 复杂度 / 可发现性债的影响？               | 体系熵增           |
| ④   | 使用者价值           | 真实使用场景、频次、严重度如何？                               | 僵尸技能           |
| ⑤   | 生命周期 TCO         | 维护 / 更新 / 废弃成本由谁承担？                               | 长期负债           |
| ⑥   | 来源可信度与许可     | 作者可信度、许可条款、商用与再分发限制？                       | 合规风险           |
| ⑦   | 战略契合             | 是否对齐 **PM + PM 双轨知识库主线**（产品经理 + 项目经理，项目经理轨含一建分支，法律 / 合同 / 工程保单风险转移为两轨交叉事项）？ | 偏离主线           |
| ⑧   | 治理自反             | 本次评估是否暴露 GOV 自身盲点（未覆盖维度、原则间冲突）？      | 治理盲区累积       |

**配套纪律**：

- 强制配套执行 `S-072`（外部来源多视角评估 SOP）的 8 镜头模板与判定口径
- 评估结论（8 镜头逐项判定 + 过评 / 不过评）记入 SOURCE-REGISTRY 对应 SRC 条目的引入流程段
- 不过评、或任一镜头标"不可入库级"红牌者，一律不得入库；存疑项标 **`待核实`** 并单独立项跟踪（对齐不杜撰 / 不确定必标注的既有纪律）
- 与原则十（外部技能通用化）衔接：通用化只在"**已决定引入且过评**"之后触发，先过评、后通用化

**治理自反（元治理）触发条款**：GOV 自身并非免检。下列任一条件触发一次 GOV 自反评审：

1. 某次引入评估的镜头 ⑧ 标出 GOV 盲点（本原则未覆盖的评估维度、或原则之间出现冲突）
2. 每 **+20 个技能** 或每 **6 个月**例行回顾（季度自查 + 半年度深查，对齐 Review Cadence）
3. 发生 skill 合并 / 拆分导致**可发现性回归**（使用时再次"一抹黑"找不到）

评审产出记入 `DECISION-LOG.md`（决策）+ `CHANGELOG.md`（变更），复用既有四记录，**不新建平行元治理系统**。若自反发现需新增原则，走原则八「修改前置治理判定」（改治理原则 vs 改技能由人类裁决）。

### 后续治理计划

1. **暂不改现有workflow-chains**：等PDF全部内容梳理完成后，统一处理workflow-chains的质量检查点和动态路由
2. **新建Skills按新标准构建**：以上11条原则作为新建Skill的硬性要求
3. **现有Skill补齐**：原则六（章节规范）和原则七（分层标注）允许在下次版本升级时逐步落地，不要求一次性全改
4. **原则八为即时生效**：2026-08-31 起所有技能修改（含进行中的三文件瘦身）均须先输出治理判定
5. **原则十一为即时生效**：2026-09-21 起所有外部来源引入（含 SRC 系列）均须按 8 镜头过评后方可入库，未过评不得入库
6. **定期回顾**：每完成一批Skill构建，回顾原则的落地情况；GOV 自反按原则十一触发条款执行
...
| 版本 | 日期       | 变更内容                                                     | 作者             |
| :--- | :--------- | :----------------------------------------------------------- | :--------------- |
| v3.7 | 2026-09-21 | **客观多视角评估 + 元治理：** §10 新增原则十一「客观多视角评估（引入前置）」（外部来源入库前禁止单一吸收视角，须按 8 镜头评估：技术正确性独立核验 / 外部基准对比 / 融合二阶风险 / 使用者价值 / 生命周期TCO / 来源可信度与许可 / 战略契合对齐 PM+PM 双轨知识库主线 / 治理自反；结论记入 SOURCE-REGISTRY，未过评不得入库），并内嵌**治理自反（元治理）触发条款**（镜头⑧标盲点 / 每+20技能或6个月 / 合并拆分致可发现性回归 → 触发 GOV 自反评审，产出记 DECISION-LOG + CHANGELOG，不建平行系统）；配套新建 S-072 外部来源多视角评估 SOP；后续治理计划 10 条原则更新为 11 条并修正重复编号 | 李志君（拍板）/ Alex_Lee（固化） |
