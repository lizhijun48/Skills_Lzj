...
| **Version**          | `v3.6`                                                       | 新增原则八（修改前置治理判定）+ 原则九（模块化拆分与融合）+ 原则十（外部技能通用化） |
...
### 原则八：修改前置治理判定（Pre-modification Governance Check）

**任何对既有技能的修改**（瘦身、拆分、合并、重构、内容变更），执行前必须先输出一次**治理判定**，结论二选一、不允许省略：

| 判定类型 | 必须包含的内容 |
|---------|--------------|
| **严格符合** | 列出命中的治理条款（§条款号 / QG门禁号）及各自的符合方式 |
| **特殊情况偏离** | 列出偏离的条款 + 偏离理由（为什么该条款不适用本场景）+ 补救措施建议 |

**判定后的决策权归属**：判定结果提交人类裁决——是**修改治理原则**（框架演进，偏离合理则原则升级）还是**要求按治理原则调整方案**（技能适配，偏离不成立则方案返工）。AI 不得在未输出判定的状态下直接修改技能，也不得自行选择"改原则"路径替人类决策。

> 本原则是 §2.4 决策层边界（合并/拆分/废弃需人类确认）的**前向扩展**：从"结果决策"前移到"修改前置判断"，把"是否合规"从隐式假设变为显式交付物。

### 原则九：模块化拆分与融合（Modular Split & Fusion）

超大技能的瘦身必须走**模块化 + 流程化**路径，而非简单删减（工程实践参考阈值：SKILL.md > 40KB，非硬性门禁，见 §9.2 防止自造门禁）：

1. **拆分**：领域知识细节、完整代码块、FAQ、异常场景全集、长尾触发词下沉 `references/`（**模块化**）；工作流正文保留 ITTO 骨架 + 关键决策点 + 路由指针（**流程化**）
2. **组合调用**：拆分出的模块必须可通过业务流程（BUSINESS-FLOW-MAP / 各 suite 的 workflow-chains）**组合调用**，模块间输入输出接口显式化——拆出来但挂不上流程的模块视为拆分失败
3. **融合评估**：拆分出的模块若与既有模块**功能相近**，必须评估融合可能（对齐 QG2 功能重叠检测口径）：能融合则融合、归并入口，减少模块绝对数；不融合需在判定中给出差异理由

**验收标准**：拆分不是目的，"**可组合、可复用、可融合**"才是——一个超大技能瘦身后，其模块应能被其他技能的业务流程复用，其相近模块应已归并或给出不归并的明确说法。

### 原则十：外部技能通用化（External Skill Genericization）

**外部引入的技能**（marketplace 导入、第三方作者署名、带 IP 声明）一旦执行拆分/模块化处理，必须同步做通用化：

| 处理对象 | 通用化规则 |
|---------|-----------|
| 个性化名称 | 人设名、作者昵称、角色名 → 中性专业表述（如"财税审计专家"） |
| 显著标识 | 口号、宇宙观/人设设定、标志性符号、平台残留路径（如 `.kiro/`）→ 删除或替换为通用描述 |
| 标志性说明 | 正文不得保留第三方标志性人设说明；功能性内容（方法论、流程、工具逻辑）正常迁移 |

**配套纪律**：
- 原版完整归档至 `archive/` 或 `~/.workbuddy/backup/` 保留溯源，通用化仅施于**工作副本**——溯源链不断，工作面中性化
- 通用化视为 fork 决策，须在 DECISION-LOG.md 记录（fork 时间、原技能出处、通用化范围、上游失联后果），对齐既有决策日志惯例
- 不拆分则不处理：通用化只在"已决定拆分/模块化"时触发，不为通用化而通用化

### 后续治理计划

1. **暂不改现有workflow-chains**：等PDF全部内容梳理完成后，统一处理workflow-chains的质量检查点和动态路由
2. **新建Skills按新标准构建**：以上10条原则作为新建Skill的硬性要求
3. **现有Skill补齐**：原则六（章节规范）和原则七（分层标注）允许在下次版本升级时逐步落地，不要求一次性全改
4. **原则八为即时生效**：2026-08-31 起所有技能修改（含进行中的三文件瘦身）均须先输出治理判定
...
| 版本 | 日期       | 变更内容                                                     | 作者             |
| :--- | :--------- | :----------------------------------------------------------- | :--------------- |
| v3.6 | 2026-08-31 | **底层逻辑固化（用户口述确立）：** §10 新增原则八「修改前置治理判定」（任何技能修改前必须输出"严格符合/特殊情况偏离"二选一判定，改原则 vs 改技能由人类裁决）、原则九「模块化拆分与融合」（超大技能走模块化+流程化路径，模块须可经业务流程组合调用，功能相近模块必须评估融合）、原则十「外部技能通用化」（第三方技能拆分时个性化名称/显著标识通用化，原版归档保留溯源，fork 决策记 DECISION-LOG）；原则八即时生效，后续治理计划由 7 条原则更新为 10 条 | 李志君（口述）/ Alex_Lee（固化） |
