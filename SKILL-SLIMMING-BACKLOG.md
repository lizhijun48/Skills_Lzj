...
> **架构澄清（用户 00:25 纠正）**：技能内引用=已蒸馏数据（独立「初始数据」），保留不删不迁；law-repository 是**原文溯源/版本核对层**（可选引用），两者解耦——原文库有无对应法条，不影响蒸馏数据存在与使用。下沉≠删除。
...
**框架原则**：law-repository 骨架已就位、更新规程已定（六条）。D1~D4 是**原文库增强**（完善溯源层），**不再作为蒸馏数据的阻塞项**——蒸馏数据本就独立存在（用户 00:25 纠正）。入库后仅提供「可选溯源引用」能力，非必须。
