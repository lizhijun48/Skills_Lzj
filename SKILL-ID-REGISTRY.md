...
> 版本：v1.6.0 | 最后更新：2026-09-21
> v1.6.0 变更：纳管根目录独立技能 external-source-eval-sop——登记 S-072（外部来源多视角评估 SOP）。背景：SRC-004 引入复盘发现原评估为"对照我方差距与可借鉴点"的单一吸收视角，存在片面性；GOV 升 v3.7 新增原则十一「客观多视角评估（引入前置）」为宪法层强制要求，S-072 为其方法层配套（8 镜头模板与判定口径）。归属判定：其 SKILL.md 第八节明确"可平移至任意套件与任意外部来源类型"，属通用方法论，且部署于根目录，故按独立部署口径（参照 S-071）归入 S- 前缀，不入任何 suite 序列。
...
| S-072 | external-source-eval-sop/SKILL.md | 外部来源多视角评估 SOP | CX | [有效] | 2026-09-21 新增；GOV v3.7 原则十一的方法层配套，外部来源引入前置强制 8 镜头（技术正确性独立核验/外部基准对比/融合二阶风险/使用者价值/生命周期TCO/来源可信度与许可/战略契合PM+PM双轨主线/治理自反），未过评不得入库；与 S-071 为方法论兄弟（S-071管权威源怎么建，S-072管外部源要不要进）；与 SOURCE-REGISTRY（只登 SRC-xxx 流向）为方法—记录配套关系 |
